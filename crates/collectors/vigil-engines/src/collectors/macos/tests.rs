...
#[test]
fn a_mac_where_the_job_has_never_run_says_the_reading_is_unknown_rather_than_empty() {
    ...
    match collector.available() {
        Health::Unavailable(why) => {
            assert!(why.contains("launchd job vigil.containers"), "{why}");
            assert!(
                why.contains("/usr/local/libexec/vigil/vigil-container-dump"),
                "{why}"
            );
            assert!(
                why.contains("not the same as a Mac with no containers"),
                "{why}"
            );
            assert!(
                !why.contains("systemctl"),
                "there is no systemd on a Mac to ask: {why}"
            );
        }
        other => panic!("{other:?}"),
    }
...
#[test]
fn on_a_mac_the_collector_reads_the_directory_the_launchd_job_writes_into() {
    let collector = EnginesCollector::new(|| AT.to_string(), Watching::default());

    assert_eq!(
        collector.dump_path(Engine::Docker),
        PathBuf::from("/usr/local/var/lib/vigil/containers/docker.json")
    );
}

#[test]
fn an_engine_whose_virtual_machine_is_stopped_is_named_as_one_that_did_not_answer() {
    let at = workspace("machine-stopped");
    let mut stopped = podman::dump();
    for answer in stopped.asked.values_mut() {
        answer.state = crate::types::FAILED.to_string();
        answer.printed = String::new();
        answer.why = Some(
            "Cannot connect to Podman. Please verify your connection to the Linux system".into(),
        );
    }
    written(&at, &docker::dump());
    written(&at, &stopped);
    ...
    let collector = reading(&at);
    let read = collector.collect().expect("docker still answered");

    assert!(
        !read
            .items
            .keys()
            .any(|key| key.starts_with("podman|image|")),
        "a machine that is stopped holds its images still, and reading it as empty would \
         report every one of them as removed"
    );
    match collector.available() {
        Health::Degraded(why) => {
            assert!(why.contains("podman did not answer"), "{why}");
            assert!(why.contains("virtual machine"), "{why}");
        }
        other => panic!("{other:?}"),
    }
    let _ = fs::remove_dir_all(&at);
