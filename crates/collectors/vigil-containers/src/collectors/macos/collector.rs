...
pub(super) const IN_A_VIRTUAL_MACHINE: &str =
    "containers run inside a VM on macOS; the engines are read by containers-engines";
...
impl ContainersCollector {
    pub fn new(_now: impl Fn() -> Rfc3339 + Send + Sync + 'static) -> Self {
        ContainersCollector {
            absent: NotOnThisSystem::new("containers", IN_A_VIRTUAL_MACHINE),
        }
    }
}
