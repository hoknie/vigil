...
fn the_reading_on_disk_is_the_one_these_parsers_build_down_to_the_value() {
    if let Err(complaint) = Golden::reading("files").write_or_check(&document(&files())) {
        panic!("{complaint}");
    }
}
...
fn the_shape_on_disk_is_the_shape_these_parsers_produce() {
    if let Err(complaint) = Golden::snapshot("files").write_or_check(&Shape::of(&files()).written())
    {
        panic!("{complaint}");
    }
}
...
fn the_shape_published_to_the_console_is_the_shape_of_the_reading_published_beside_it() {
    let held = Golden::reading("files")
        .held()
        .expect("files has no reading on disk");
    ...
    assert_eq!(
        ...
        Golden::snapshot("files")
            .held()
            .expect("files has no shape on disk")
    );
}
