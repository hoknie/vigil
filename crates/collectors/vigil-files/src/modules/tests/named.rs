...
use crate::types::{Layout, Named, WATCHED_BY_DEFAULT, Watched, Watching};
...
fn a_host_whose_file_names_no_path_watches_the_list_this_product_ships() {
    let Layout::Named(watching) = Watching::default().layout() else {
        panic!("a block that names no list of its own watches the shipped one");
    };
...
fn a_host_that_names_its_own_paths_watches_those_and_not_the_shipped_ones() {
    ...
    assert_eq!(
        watching.layout(),
        Layout::Named(Named {
            paths: vec![Watched::Named("/etc/sudoers".to_string())],
