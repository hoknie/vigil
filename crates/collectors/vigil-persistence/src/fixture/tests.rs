...
fn the_reading_on_disk_is_the_one_these_parsers_build_down_to_the_value() {
    if let Err(complaint) = Golden::reading("persistence").write_or_check(&document(&persistence()))
    {
        panic!("{complaint}");
    }
}
...
fn the_shape_on_disk_is_the_shape_these_parsers_produce() {
    if let Err(complaint) =
        Golden::snapshot("persistence").write_or_check(&Shape::of(&persistence()).written())
    {
        panic!("{complaint}");
    }
}
...
fn the_shape_published_to_the_console_is_the_shape_of_the_reading_published_beside_it() {
    let held = Golden::reading("persistence")
        .held()
        .expect("persistence has no reading on disk");
    ...
    assert_eq!(
        ...
        Golden::snapshot("persistence")
            .held()
            .expect("persistence has no shape on disk")
    );
}
...
fn a_reading_that_could_not_be_finished_says_so_in_a_row_of_its_own() {
    assert!(
        persistence().items.contains_key("modules|unreadable"),
        "a host whose loaded modules could not be read is not a host with none"
    );
}
