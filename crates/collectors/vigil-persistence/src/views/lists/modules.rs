...
use super::super::fields::{number, right, size_of, strings, text};

const SIZE_COLUMN: usize = 9;

pub(crate) fn columns(_wide: bool) -> Vec<Column> {
    vec![
...
pub(crate) fn cells(key: &str, item: &Value, unreadable: bool) -> Vec<String> {
    if unreadable {
        return vec![
            "—".to_string(),
            right("—", SIZE_COLUMN),
...
pub(crate) fn held(item: &Value) -> String {
