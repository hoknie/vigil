...
fn reading() {
    ...
    println!(
        "persistence: health {:.2} ms: {health:?}",
        started.elapsed().as_secs_f64() * 1000.0
    );
}
...
fn reading() {
    println!("persistence: this reading is taken on Linux and on macOS; nothing to measure here");
}
...
fn searching() {
    let read = vigil_persistence::fixture::persistence();
