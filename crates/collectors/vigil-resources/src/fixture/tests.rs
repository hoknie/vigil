...
fn the_reading_on_disk_is_the_one_these_parsers_build_down_to_the_value() {
    if let Err(complaint) = Golden::reading("resources").write_or_check(&document(&resources())) {
        panic!("{complaint}");
    }
}
...
fn the_shape_on_disk_is_the_shape_these_parsers_produce() {
    if let Err(complaint) =
        Golden::snapshot("resources").write_or_check(&Shape::of(&resources()).written())
    {
        panic!("{complaint}");
    }
}
...
fn the_shape_published_to_the_console_is_the_shape_of_the_reading_published_beside_it() {
    let held = Golden::reading("resources")
        .held()
        .expect("resources has no reading on disk");
    ...
    assert_eq!(
        ...
        Golden::snapshot("resources")
            .held()
            .expect("resources has no shape on disk")
    );
}
