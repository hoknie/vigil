...
fn reading() {
    ...
    let collector = match vigil_resources::Resources.collector(&settings) {
        Ok(collector) => collector,
        Err(why) => {
            println!("resources: {why}");
            return;
        }
    };
    ...
    let Ok(first) = collector.collect() else {
        println!(
            "resources: nothing to read here: {:?}",
            collector.available()
        );
        return;
    };
    println!(
        "resources: first reading {:.2} ms, {} items, {} bytes as the baseline",
    ...
    println!(
        "resources: {:.2} ms per reading over {ROUNDS}",
...
fn reading() {
    println!("resources: this reading is taken on Linux and on macOS; nothing to measure here");
}

fn drawing() {
    let reading = vigil_resources::fixture::resources();
    let room = Room::of(160);

    println!("pane          rows ms  cells ms (a window of {WINDOW})");
    for pane in vigil_resources::TheHostAndItsFiles.panes() {
...
fn searching() {
    let read = vigil_resources::fixture::resources();
