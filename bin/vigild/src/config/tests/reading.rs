...
fn shipped_into(root: &std::path::Path, directory: &std::path::Path, from: &std::path::Path) {
    std::fs::create_dir_all(directory).expect("a directory");
    for entry in std::fs::read_dir(from).expect("the shipped configuration is readable") {
        let entry = entry.expect("an entry");
        let target = directory.join(entry.file_name());
        if entry.path().is_dir() {
            shipped_into(root, &target, &entry.path());
            continue;
        }
        let text = std::fs::read_to_string(entry.path()).expect("readable");
        std::fs::write(
            &target,
            text.replace("/etc/vigil/", &format!("{}/", root.display())),
        )
        .expect("write");
    }
}
...
fn the_file_this_product_ships_is_a_file_this_product_reads() {
    let directory = bench("shipped");
    let shipped = std::path::Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/../../config"));
    shipped_into(&directory, &directory, shipped);

    let config = load(
        directory
            .join("vigil.example.yaml")
            .to_str()
            .expect("utf-8"),
    )
    .expect("the shipped example must load");

    let every: Vec<String> = crate::modules::names()
        .iter()
    ...
    assert_eq!(
        ...
        "the shipped example keeps its collectors in a directory of their own"
    );
    assert_eq!(
        config.collectors,
        Some(every.clone()),
        "the shipped example switches on every collector this product has, in the product's order"
    );
    for name in &every {
        assert!(
            config.apart.block(name).is_some(),
            "{name} has no block in the shipped collectors, so an operator has nowhere to find \
             its period or its settings"
        );
    }
    assert!(
        !config.killing.from_the_console
            && !config.accounts.from_the_console
            && !config.units.from_the_console,
        "the shipped files switch on nothing the console could do to a host"
    );
}
...
fn a_collector_named_twice_is_refused_rather_than_watched_twice() {
    ...
    std::fs::write(&path, "collectors: [network, users, network]\n").expect("write");
