...
use vigil_firewall::{ANSWERED, Answer, FAILED, Question, TIMED_OUT};
...
pub fn ask(
    ...
    let printed = scratch(at, turn, "out");
    let complained = scratch(at, turn, "err");
    let started = Instant::now();

    let child = start(question, &printed, &complained);
    let mut child = match child {
        Ok(child) => child,
        Err(why) => {
            let _ = fs::remove_file(&printed);
            let _ = fs::remove_file(&complained);
            return unfinished(question, FAILED, started, None, Some(why));
        }
    };
    ...
    match state {
        Some(Some(0)) => Answer {
            ...
            program: question.program.to_string(),
            arguments: question.arguments.clone(),
        ...
        Some(status) => unfinished(question, FAILED, started, status, why),
        None => unfinished(question, TIMED_OUT, started, None, why),
    }
}

fn start(question: &Question, printed: &Path, complained: &Path) -> Result<Child, String> {
    ...
    Command::new(question.program)
        ...
        .env("LC_ALL", "C")
        .spawn()
        .map_err(|error| format!("{} could not be run: {error}", question.program))
}
...
fn scratch(at: &Path, turn: usize, extension: &str) -> PathBuf {
    at.join(format!(
        ".vigil-firewall-dump.{}.{turn}.{extension}",
        std::process::id()
    ))
}
