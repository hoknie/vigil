...
impl App {
    ...
    pub(super) fn button_at(&self) -> Option<usize> {
    ...
    pub(super) fn deeper_into_the_buttons(&mut self) -> bool {
    ...
    pub(super) fn back_out_of_the_buttons(&mut self) -> bool {
    ...
    pub(super) fn rest_the_buttons(&mut self) {
        self.button = 0;
        self.in_the_buttons = false;
    }

    pub(super) fn press_the_button(&mut self) -> bool {
    ...
    pub(super) fn unmark_everything(&mut self) -> bool {
        if !self.marking_offered() {
            self.message = Some(NOTHING_TO_MARK.to_string());
            return false;
        }
    ...
    pub(super) fn show_the_suppressions(&mut self) -> bool {
        if !self.marking_offered() {
            self.message = Some(NOTHING_TO_MARK.to_string());
            return false;
        }
    ...
    pub(super) fn finding_key_of(&self, key: &str) -> Option<String> {
        let pane = self.pane()?;
        let Reading::Taken(snapshot) = self.view.reading(pane.reads()) else {
            return None;
        };
