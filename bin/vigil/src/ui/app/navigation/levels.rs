...
impl App {
    ...
    pub(in crate::ui::app) fn remember_section(&mut self, screen: Screen) {
        let keys = home::keys(&self.view);
        self.nav.sections.point_at(screen.name(), &keys);
    }

    pub(in crate::ui::app) fn section_under_cursor(&self) -> Option<home::Row> {
        home::rows(&self.view)
            .into_iter()
            .nth(self.nav.sections.at())
    }

    pub(in crate::ui::app) fn go_back(&mut self) {
        self.go_back_from(true);
    }

    pub(in crate::ui::app) fn leave_the_row(&mut self) {
    ...
    pub(in crate::ui::app) fn open(&mut self) {
        if self.nav.at() == Screen::HOME {
            self.open_section();
            return;
        }
    ...
    pub(in crate::ui::app) fn close_the_branch(&mut self) -> bool {
        let Some(row) = self.pane_row_under_the_cursor() else {
            return false;
        };
        if row.opens() && row.opened {
    ...
    pub(in crate::ui::app) fn sideways(&mut self, by: isize) {
        if self.level == Level::Detail {
